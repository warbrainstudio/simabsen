  ...
  $(document).ready(function() {
    ...
    if ($("#" + _table)[0]) {
      var table_mesin = $("#" + _table).DataTable({
        ...
        serverSide: true,
        ajax: {
          url: "<?php echo base_url('mesin/ajax_get_all/') ?>",
          type: "get"
        },
        columns: [{
            data: null,
    ...
    $("#" + _table).on("click", "a.action-check-connect", function(e) {
        ...
        $.ajax({
            type: "get",
            url: "<?php echo base_url('mesin/ajax_check/') ?>" + ip,
    ...
    $("#" + _modal + " ." + _section + "-action-save").on("click", function(e) {
      ...
      $.ajax({
        type: "post",
        url: "<?php echo base_url('mesin/ajax_save/') ?>" + _key,
        data: $("#" + _form).serialize(),
        success: function(response) {
          var response = JSON.parse(response);
          if (response.status === true) {
    ...
    $("#" + _table).on("click", "a.action-delete", function(e) {
      ...
      swal({
        ...
        if (result.value) {
          $.ajax({
            type: "delete",
            url: "<?php echo base_url('mesin/ajax_delete/') ?>" + temp.id,
            dataType: "json",
            success: function(response) {
              if (response.status) {
                $("#" + _table).DataTable().ajax.reload(null, false);
