...
<!-- PDFObject -->
<script src="<?php echo base_url('themes/_public/js/pdfobject.min.js') ?>"></script>
...
<script type="text/javascript">
	...
	$(document).on("click", ".x-load-partial", function(e) {
		e.preventDefault();
		...
		$.ajax({
			...
			success: function(response) {
				$("#app-main-content").empty();
				$("#app-main-content").html(response);
				hideBodyLoading();
			},
			error: function() {
				hideBodyLoading();
	...
	$(document).on("click", ".x-load-modal-partial", function(e) {
		e.preventDefault();
		var partialUrl = $(this).attr("href");
		var modalId = $(this).attr("modal-id");

		// Remove existing DOM
		$(`div[id=${modalId}]`).remove();
		$("#app-main-content-modal-partial").empty();
		...
		$.ajax({
			...
			success: function(response) {
				$("#app-main-content-modal-partial").html(response);
				$(`#${modalId}`).modal().show();
	...
	// Load modal swal view with ajax
	$(document).on("click", ".x-load-swal-content", function(e) {
		e.preventDefault();
		var partialUrl = $(this).attr("href");
		var iframeRef = $(this).attr("iframe-id");
		...
		$.ajax({
			...
			success: function(response) {
				var dom = `<iframe id="iframe-swal" src="data:text/html, ${encodeURIComponent(response)}" frameborder="0" style="width: 100%; height: 420px;"></iframe>`;
				Swal.fire({
	...
	// Load modal partial view with ajax
	function xLoadModalPartial(partialUrl, modalId) {
		// Remove existing DOM
		$(`div[id=${modalId}]`).remove();
		$("#app-main-content-modal-partial").empty();
	...
	// Load modal swal in parent doc
	function swalPartial(title, text, type, exec) {
		swal({
			title: (title) ? title : '',
			text: (text) ? text : '',
	...
	// Load notify in parent doc
	function notifyPartial(text, type) {
		notify((text) ? text : '(Message not set)', (type) ? type : 'success');
	};
</script>

<!-- For modal partial content -->
<div id="app-main-content-modal-partial"></div>
</body>
